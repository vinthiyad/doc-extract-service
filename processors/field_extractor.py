...
def extract_fields(
    ...
    if doc_type == "invoice":
        try:
            ...
            if os.path.isdir(template_dir):
                ...
                # invoice2data requires a file path, not raw text.
                # We'll rely on the regex fallback below.
        except ImportError:
    ...
    gstin_match = re.search(
        r"\d{2}[A-Z]{5}\d{4}[A-Z]{1}[A-Z\d]{1}Z[A-Z\d]{1}", text
    )
    if gstin_match:
    ...
    if fields.total_amount is None:
        ...
        if all_numbers:
            try:
                fields.total_amount = max(
                    float(n.replace(",", "")) for n in all_numbers
                )
            except ValueError:
                pass
